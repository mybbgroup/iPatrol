<?php

$l['locate_title'] = "Location Details";
$l['locate_item'] = "Field";
$l['locate_detail'] = "Value";
$l['locate_button'] = "[Locate]";
$l['permission_error'] = "You are not allowed to perform this action.";
$l['invalid_response'] = "An invalid response received from API. Please report to admin.";
$l['private_range'] = "The IP address is considered falling under private range and blocked by IANA from public usage.";
$l['reserved_range'] = "The IP address is considered falling under reserved range.";
$l['invalid_query'] = "The IP address / query made to API is invalid.";
$l['spampost_caught'] = "Your post appears to be suspicious and requires a moderator approval to be visible.";
$l['spamthread_caught'] = "<br><b>However; your thread appears to be suspicious; kept under moderation and will only be visible after a moderator approves it.</b>";

$l['api_as'] = "Autonomous System:";
$l['api_isp'] = "Service Provider:";
$l['api_reverse'] = "Reverse DNS:";
$l['api_lat'] = "Latitude:";
$l['api_lon'] = "Longitude:";
$l['api_zip'] = "Zip Code:";
$l['api_regionName'] = "Region:";
$l['api_city'] = "City:";
$l['api_country'] = "Country:";
$l['api_timezone'] = "Timezone:";
$l['api_org'] = "Organization:";

$l['event_subject'] = "{1} at {2}";
$l['event_bottrap'] = "New spider visit detected";
$l['event_actblock'] = "Action blocked";
$l['event_ipban'] = "IP address banned";

$l['event_1'] = "The detected spider's name is similar to {1} existing spider/s.";
$l['event_2'] = "This is for your information and further necessary manual action.";
$l['event_3'] = "The detected spider is added to the database by iPatrol.";
$l['event_8'] = "An attempt of a visit using proxy has been detected from IP address {1}.\r\nThe mentioned IP address has automatically been banned by iPatrol.";

$l['matter_body'] = "Hello,\r\n\r\n{1}\r\n\r\nWishes,\r\n- iPatrol\r\nhttps://mybb.group";
$l['matter_bottrap'] = "A new spider visit has been detected.\r\nNew spider is identified as '{1}' having a user agent string '{2}'.\r\n";
